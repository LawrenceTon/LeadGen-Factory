...
class InspectorView(ctk.CTkFrame):
    def __init__(self, master, logic):
        ...
        self.entry_timeout = ctk.CTkEntry(self.limits_frame, placeholder_text="Timeout (sec)")
        self.entry_timeout.insert(0, "30")
        self.entry_timeout.pack(pady=5, padx=5)
        
        self.btn_export = ctk.CTkButton(self.limits_frame, text="Export History", command=self.export_history, fg_color="#3B8ED0")
        self.btn_export.pack(pady=10, padx=5)

        self.btn_reset = ctk.CTkButton(self.limits_frame, text="♻️ Reset DB", command=self.reset_database_ui, fg_color="#95a5a6")
    ...
    def export_history(self):
        ...
        if success:
             print(f"📂 FILE SAVED: Look for '{os.path.basename(path)}' in your project folder.")
             os.startfile(os.getcwd())

    def reset_database_ui(self):
        if self.logic.reset_database():
            self.log_msg("History Cleared. Database reset successfully.")
            self.progress_bar.set(0)
            self.lbl_status.configure(text="Status: DB Reset")
        else:
            self.log_msg("Error: Failed to reset database (File might be in use).")
...
class LeadGenApp(ctk.CTk):
    def __init__(self):
        ...
        try:
            ...
            pass # Icon might not be present during dev
